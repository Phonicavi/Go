...
int Go::generate_move(bool early_pass)
{
  ...
  while (clock() - fin_clock < max_time) {
    ...
    if (node->get_visits() >= EXPAND || node == root) {
      ...
      if (main_goban->get_history_length()<170)main_goban->init_priors(priors);
      tree.expand(node, legal_moves, nlegal, priors); //TODO: break if expand fails.
    }
    node_history[nnode_hist++] = node;
    int result;
    if (main_goban->get_history_length()<20) result = play_random_game(LIGHT);
    else  result = play_random_game(HEAVY);//Black wins.
#ifdef DEBUG_INFO
      main_goban->print_goban();
      std::cerr << result << "\n";
#endif
