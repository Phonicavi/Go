...
{
  // static FastLog fastLog(10);

  if (visits) {
    if (r_visits) {
     double beta = r_visits/(r_visits + visits + r_visits*visits*BIAS);
      // double beta = sqrt(K/(3*visits+K));
      return (1.0-beta)*results/visits + beta*r_results/r_visits;//+UCTK*sqrt(log10(parent_visits)/visits);
    } else {
      // std::cerr << 5 <<std::endl;
      return ((double)results)/visits;//+UCTK*sqrt(log10(parent_visits)/visits);
    }
  } 
  // else if (r_visits) {
  //     return r_results/r_visits;
  // }
  return rand()*1000+10000;   //first player urgency.
}
