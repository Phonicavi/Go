...
// int Board::set_fixed_handicap(int newhandicap)
...
// int Board::set_handicap(const int handicap[])
// {  
...
void Board::merge_neighbour(int point,GroupSet<4> * groupset, Group * Greatgroup){
    ...
    for (int i=0; i<len; ++i){
      ...
      if (cur_gr!=Greatgroup){
        ...
        for (Group::StoneIterator st(cur_gr); st; ++st){
          points[*st] = Greatgroup;
        }
        if (REAL){
            unordered_set<int>::iterator it;
...
int Board::play_move(int point)  //this move isn't stored in game_history.
{
  if (point) {
...
bool Board::set_position(const PList &moves)
{
...
#ifdef ZOBRIST
unsigned long long Board::get_zobrist() const
{
  return zobrist.get_key();
}

unsigned long long Board::get_zobrist(int move)const
{
...
unsigned long long Board::set_zobrist(int move) const
{
  if (move) {
...
#endif
int Board::point_liberties(int point, PList &liberties) const
...
int Board::neighbour_groups(int point, GroupSet<4> &neighbours, Group *Greatgroup) const
{
  ...
  for (int i = 0; adjacent[point][i]; i++) {
    neighbours.add(points[adjacent[point][i]]);
    if (points[adjacent[point][i]]->get_nstones() > maxLen){
      maxLen = points[adjacent[point][i]]->get_nstones();
      Greatgroup = points[adjacent[point][i]]; 
    }
  }
  return neighbours.length();  
}
...
int Board::get_value(int point) const   //Value 1 for Black, -1 for White, 0 for empty.
{
  if (points[point]) {
...
float Board::score_count() const
{
  ...
  for (int i = 1; i <= size2; i++) {
    if (points[i]) {
      // if (points[i]->get_color()) --ret;
      // else ++ret;
      ret += (-(points[i]->get_color()<<1)+1);
    } else {
...
void Board::record_goban(){
  ...
  for (int y = size - 1; y > -1; y--) {
    ...
    for (int x = 1; x < size+1; x++) {
      if (points[size*y + x]) {
        if (points[size*y + x]->get_color()) ss  << "#";
        else ss  << "o";
  ...
  ss<< "\n";
    ...
    ss << "  |";
    for (int x = 1; x < size+1; x++) {
      ss << (points[size*y+x]?points[size*y+x]->get_nliberties():0) << " ";
    }
    ss  << "|" << y+1 << "\n";
  }
  ss.close();
